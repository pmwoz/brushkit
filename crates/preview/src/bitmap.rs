...
fn jpeg_header(bytes: &[u8]) -> Result<JpegHeader, zune_jpeg::errors::DecodeErrors> {
    ...
    // zune-jpeg reads a frame's sides as u16.
...
///
/// zune-jpeg keeps its frame header private, and it rejects a second one, so
/// every 0xFFC2 marker whose header has a consistent length and the size
/// zune-jpeg reports is a candidate, and the largest count wins. zune-jpeg
/// decodes progressive only after a 0xFFC2 frame header, so the count is never
/// below zune-jpeg's, and is 0 for a baseline JPEG without a candidate.
fn progressive_coefficient_bytes(bytes: &[u8], width: u32, height: u32) -> u64 {
    (1..bytes.len())
        ...
        .filter_map(|i| {
            ...
            if field(0) as usize != length || field(3) != height || field(5) != width {
                return None;
            }
